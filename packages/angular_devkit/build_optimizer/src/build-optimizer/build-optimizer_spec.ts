...
describe('build-optimizer', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('doesn\'t process files without decorators/ctorParameters/outside Angular', () => {
      ...
      expect(oneLine`${boOutput.content}`).toEqual(input);
  ...
  describe('sourcemaps', () => {
    ...
    it('doesn\'t produce sourcemaps for ignored or invalid files', () => {
      ...
      const invalidInput = oneLine`
        ...
        Clazz.decorators = [ { type: Injectable } ];
      `;
      expect(buildOptimizer({ content: ignoredInput, emitSourceMap: true }).sourceMap).toBeFalsy();
      expect(buildOptimizer({ content: invalidInput, emitSourceMap: true }).sourceMap).toBeFalsy();
    });
