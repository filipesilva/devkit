...
describe('Eject Schematic', () => {
  ...
  it('should template entry points', () => {
    const tree = schematicRunner.runSchematic('eject', defaultOptions, workspaceTree);
    const webpackFileContent = tree.readContent('/webpack.config.factory.js');
    expect(webpackFileContent).toContain(tags.trimNewlines`
    entry: {
    ...
    `);
    const karmaFileContent = tree.readContent('/webpack.karma.conf.js');
    expect(karmaFileContent).toContain(tags.trimNewlines`
    files: [
