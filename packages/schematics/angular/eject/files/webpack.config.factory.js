...
function webpackConfigFactory(options) {
  ...
  const { sourceMap, optimize, test } = options;

  // Project and workspace paths.
  const workspaceRoot = path.resolve(__dirname, '<%= relativePathToWorkspaceRoot %>');
...
<%= stylesTpl %>
  ...
  return {
    ...
    devtool: test ? 'inline-source-map' : optimize ? false : 'source-map',
    context: projectRoot,
    resolve: {
      extensions: ['.ts', '.js']
    },
    entry: {
      ...
      <% if (polyfills) { %>polyfills: path.resolve(workspaceRoot, '<%= polyfills %>'),<% } %>
      styles: globalStylesEntryPoints,
    },
    output: {
      path: path.resolve(workspaceRoot, '<%= outputPath %>'),
      filename: '[name].js',
    },
    optimization: {
      noEmitOnErrors: true,
      // Runtime chunk configuration. Don't create a runtime chunk when testing.
      runtimeChunk: test ? false : 'single',
      ...
      // Chunk splitting configuration. Don't split chunks when testing.
      splitChunks: test ? false : {
    ...
    plugins: [
      ...
      new AngularCompilerPlugin({
        tsConfigPath: path.resolve(workspaceRoot,
          test ? '<%= testTsConfig %>' : '<%= tsConfig %>'
        ),
      ...
      new IndexHtmlWebpackPlugin({
        input: path.resolve(workspaceRoot, '<%= index %>'),
        entrypoints: <%= entryPointsTpl %>,
      }),
      // Asset configuration.
